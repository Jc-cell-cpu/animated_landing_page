...
function Apps({}: Props) {
  ...
  return (
    <motion.div ref={scrollref} style={{opacity , translateX}} className='relative my-4'>
        <div className='relative z-10 w-full max-w-screen-xl mx-auto px-4 sm:px-8 lg:px-1 py-40 grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-16 lg:gap-x-40'>
            <div className='flex flex-col items-center gap-8'>
                <p className='text-white'>JC works everywhere:</p>
                <div className='flex w-full justify-around md:justify-between'>
                    ...
                    <Image className='h-10 w-auto sm:h-12 saturate-0 hover:saturate-150 transition-all duration-500 cursor-pointer' src='/deno.png' alt='Node' width={114} height={102}/>
        ...
        <div className='absolute inset-x-0 bottom-0 h-96 overflow-hidden'>
            <div className='absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 h-[50px] w-full bg-violet-500/30 blur-3xl'></div>
            <div className='absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 h-[75px] w-2/3 bg-rose-500/40 blur-3xl'></div>
        </div>
        <hr className='w-full border-none h-px bg-gradient-to-r from-rose-700/0 via-violet-700/25 to-rose-700/0' />
    </motion.div>
  )
}
